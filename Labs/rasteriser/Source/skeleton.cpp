...
void VertexShaderP( vec4& v, Pixel& p);
...
void DrawPolygonRowsPixel(screen* screen, vector<Pixel>& leftPixels, vector<Pixel>& rightPixels);
...
//projects 3D point v to 2D image plane storing co-ords in given p int vector
void VertexShaderPixel( vec4& v, Pixel& p){
  vec4 pp;
  //update rotation matrix R from keyboard input
  updateRotationMatrix(cumYaw);
  //calculate rotated and transformed matrix pp
...
void InterpolatePixel(Pixel a, Pixel b, vector<Pixel>& result){
  ...
  for(int i=0; i<N; ++i){
    ...
    //setting depth buffer value
    depthBuffer[result[i].y][result[i].x] = current.zinv;
    current.x += xStep;
    current.y += yStep;
    current.zinv += zStep;
...
//fucntion receives array of projected vertices vertexPixels of a given shape
//function interpolates between points, outputting furthest left and right pixels into arrays
void ComputePolygonRowsPixel(vector<Pixel>& vertexPixels,vector<Pixel>& leftPixels,vector<Pixel>& rightPixels){
  // 1. Find max and min y-value of the polygon
  //    and compute the number of rows it occupies.
  int maxY = -numeric_limits<int>::max();
  int minY = numeric_limits<int>::max();
  ...
  for(int i = 0; i < vertexNo; i++){
    ...
    //difference.zinv = abs(vertexPixels[i].zinv - vertexPixels[j].zinv);

    //find number of rows between two vertices (different to ROWS)
    // int intArraySize = abs(vertexPixels[i].y - vertexPixels[j].y) + 1;
    //allocate vector array for interpolated co-ords
    int intArraySize = difference.y + 1;
...
void DrawPolygonRowsPixel(screen* screen,  vector<Pixel>& leftPixels,  vector<Pixel>& rightPixels){
  if(rightPixels.size() != leftPixels.size()){
    ...
    for(uint i = 0; i < leftPixels.size(); i++){
      DrawLineSDLPixel(screen, leftPixels[i], rightPixels[i], currentColour);
    }
  }
}
...
void DrawPolygon(screen* screen,  vector<vec4>& vertices){
  ...
  DrawPolygonRowsPixel(screen, leftPixels, rightPixels);

}

//where fill DB
//does drawpol take pixel
//where clear depth buffer
//drawing where to compare
//how to change ComputePolygonRows
